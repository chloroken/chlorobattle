...
func _ready() -> void:
	...
	currentBoard = get_parent().get_parent().get_parent()
...
func _physics_process(delta: float) -> void:
	...
	var boardRadius = currentBoard.boardRadius
	if $DisableBounceTimer.is_stopped():
		
		# Ensure we bounce at the right location
		if distFromCenter > boardRadius: # position.distance_to(destination) < 10 || 
