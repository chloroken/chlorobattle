...
func accuracy_phase(attackingPawn, attackerUsername) -> bool:
	...
	if hitRoll > hitChance:
		...
		combat_log("[" + str(attackerUsername) + "] drunkenly missed [" + str(self.username) + "]")
		return(false)
...
func damage_phase(finalDmg, attackingPawn, attackerUsername, body) -> void:
	...
	combat_log("[" + str(attackerUsername) + "] hit [" + str(self.username) + "] for " + "%0.2f" % finalDmg + " (" + str(body.attackName) + ")")
